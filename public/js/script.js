...
socket.on('gamesOnline', function(data){
    ...
    while (i < 9) {
        ...
        $('.games .games__item:eq('+i+') .games__item-text p').html(Number(onlineG)+' players');
...
function stretchArray(arr, limit) {
  if (!arr.length || limit < arr.length) {
    throw new Error('Failed to stretch array.');
  }

  // // Integer by which each element will be multiplied.
  const integer = Math.floor(limit / arr.length);

  // // a % b - remainder from division.
  // // We will add one to each element.
  // // Whoever got it, got it, like crumbs from the table.
  let crumbs = limit % arr.length;

  const newArr = Array.from(arr, function(item, index) {
    // // How many times to repeat the array element.
    const repeater = integer + (crumbs > 0 ? 1 : 0);

    // // Decrease the number of crumbs.
    crumbs--;

    // // Repeat array element and return.
...
function betWheel(coff){
	if(window.BetOffWheel == 'off'){
		notification('error','Bets closed, wait for next round')
...
function changeRepostBalance(){
	$.post('/repost/change',{_token: csrf_token}).then(e=>{
		if(e.success){
			$('#bonusBalance').html(0)
			notification('success', 'Bonus balance successfully transferred to main balance')
			balanceUpdate($('#balance').attr('balance'), e.balance)      
		}else{
...
function changeRefBalance(){
	$.post('/refs/change',{_token: csrf_token}).then(e=>{
		if(e.success){
			$('#refBalance').html(0)
			notification('success', 'Referral balance successfully transferred to main balance')
			balanceUpdate($('#balance').attr('balance'), e.balance)      
		}else{
			notification('error', e.mess)
		}
...
function createPromoUser(that) {
	$.post('/promo/create',{_token: csrf_token, name: $('#name_crpromo').val(), sum: $('#sum_crpromo').val(), act: $('#act_crpromo').val()}).then(e=>{
		...
		if(e.success){
			balanceUpdate(e.lastbalance, e.newbalance)
			notification('success', 'Promo code created successfully') 
			close_modal()
		}else{
			notification('error', e.mess)
		}
...
function actPromo(that){
	...
	$.post('/promo/act',{_token: csrf_token, name: $('#promo_name').val()}).then(e=>{
		...
		if(e.success){
			balanceUpdate(e.lastbalance, e.newbalance)
			notification('success', 'Promo code activated successfully')
			$('#promo_name').val('')
			captcha_r()
...
function canselWithdraw(id, that) {
	$.post('/withdraw/cansel',{_token: csrf_token, id}).then(e=>{
		...
		if(e.success){
			balanceUpdate(e.lastbalance, e.newbalance)
			notification('success', 'Withdrawal cancelled successfully')
...
function goWithdraw(that) {
	$.post('/withdraw/go',{_token: csrf_token, sum: Number($('#sum_withdraw').val()), system: $('#systemW').val(), wallet: $('#wallet_withdraw').val()}).then(e=>{
		...
		if(e.success){
			balanceUpdate(e.lastbalance, e.newbalance)
			notification('success', 'Withdrawal request submitted. Processing time: 5 minutes to 24 hours')
			...
			$('.wallet__history--withdraw').prepend('<div class="wallet__history-item d-flex justify-space-between align-center">\
                    ...
                    <div id="statusW_'+e.withdraw.id+'" class="wallet__history-status  warning">\
                        <span >Pending... (<a onclick="disable(this);canselWithdraw('+e.withdraw.id+', this)">Cancel</a>)</span>\
                    </div>\
                </div>')
		}else{
			notification('error', e.mess)
...
function goDeposit(that) {
	$.post('/deposit/go',{_token: csrf_token, sum: Number($('#sumDep').val()), system: $('#systemDep').val(), promo: $('#promoDep').val()}).then(e=>{
		...
		if(e.success){
			...
			if(e.modal == 0){
				notification('success', 'Redirecting to payment')
...
$(document).ready(function(){ 
  ...
  $(".click_pp").click(function(){ // Toggle profile panel on button click
      $(".btn_open_close_prof").toggleClass('active'); // Toggle active class on profile div
  });

   $(".btn_open_close_prof").focusout(function(){ // Handle focus out event
	     $(".btn_open_close_prof").removeClass('active'); // Remove active class when focus is lost
	 });
    $("#messageChat").focus(function(){ // Handle chat input focus
	     $(".panel_smiles").removeClass('open'); // Close emoji panel
	     $(".panel_stickers").removeClass('open'); 
	     
	 });

  $(".chat_btn").click(function(){ // Toggle chat panel
  	$(".col-chat-lg").toggleClass('open'); 
  	$(".chat_btn").toggleClass('active');
  	chatScroll()
  });

  $(".more_btn").click(function(){ // Toggle button click handler
  	$(".panel_more").toggleClass('open'); 
  	$(".more_btn").toggleClass('active');
  });

   $(".bets_btn").click(function(){ // Toggle button click handler
...
function addDemoBalance(){
	$.post('/add/demobalance',{_token: csrf_token, addbalance: $('#add_balance').val()}).then(e=>{
		if(e.success){
			balanceUpdate($('#balance').attr('balance'), e.balance)
			$('.close').click();
			notification('success', 'Demo balance added successfully')
		}else{
			notification('error', e.mess)
		}
	});
...
function updateMinesXNew(){
	...
	for(let i = 0; i< level - BombMines; i++){
		...
		$('.mines__scroll').append('<div class="mines__x-item">\
			<p class="d-flex align-center justify-space-between">'+Number(i + 1)+' step <span>'+coeffMine.toFixed(2)+'x</span></p>\
			<b>'+win.toFixed(2)+'</b>\
			</div>')
	}
}
	...
	$.post('/newmines/start',{_token: csrf_token,level: $('#LevelMines').val(), bet: Number($('#BetMines').val()), bomb: Number($('#BombMines').val())}).then(e=>{
		if(e.success){
			...
			if(e.bonus == 1){
				...
				setTimeout(() => notification('success', 'Congratulations! Your bet multiplied by '+e.bonusMine),10000);
...
function addInBets(jackpot){
	jackpot.forEach((e)=>{
		...
		$('.betsJackpot').prepend('<div class="betJackpot" data-user_id="'+e.user_id+'">\
			...
			<span class="nameTicket">Ticket</span>\
...
socket.on('laravel_database_kenoWin',e => {
	...
	if(USER_ID == e.user_id){
		...
		$('.keno__mines .keno__mines-win b').html((Number(e.win).toFixed(0))+' ?');
	}
})
...
function selectKeno(id, that){
	if(selectsKeno.indexOf(id) != -1){  
		...
		if (selectsKeno.length == 5){
			return notification('error', 'Minimum 5 coins')
		}
		selectsKeno.push(id)
...
$('#lightTheme').click(function(e){
 ...
 function crashBet(that){
 	...
 	$.post('/crash/bet',info).then(e=>{
 		if(e.success){
 			...
 			notification('success',e.success)
 			balanceUpdate(e.lastbalance, e.newbalance)
 			$('#btnCrash span').html('Pending game...')	
 			disable('#crashSum')	
 			disable('#crashAuto')				
 		}
 		if(e.error){
 ...
 function crashGive(that){
 	...
 	$.post('/crash/give',info).then(e=>{
 		if(e.success){
 			bet_user = 0;
 			notification('info', 'forTicket...')
 			// $('#btnCrash').html('forTicket...')
 		}
 		if(e.error){
 			undisable(that)
 			notification('error',e.error)
 	...
 	$.post('/coin/bet',{_token: csrf_token, bet: $('#coinSum').val()}).then(e=>{
 		...
 		if(e.success){	
 			...
 			if(e.bonus == 1){
 				...
 				setTimeout(() => notification('success', 'Congratulations! Your bet multiplied by '+e.coeffBonusCoin),10000);
