...
function searchMultUser(id){
	$.post('/admin/searchmultuser', {_token: csrf_token, id: id}).then(e=>{
		if(e.mults.length > 0){
			$('.mult_info_user').removeClass('bg_success').html('Multi-accounts detected').addClass('bg_danger')
		}
		e.mults.forEach((e)=>{
			if(e.ban == 0){
				ban = 'lock'
...
function closeEditSystemWithdraw(){
	$('#title_s_w').html('to??in???and? ?and???? in?in?Yes')
	$('#img_val').val('');
	imgWithdraw();
	$('#min_w').val('50');
	$('#comm_percent').val('5')
...
function editSystemWithdraw(id, name, img, comm_percent, comm_rub, min_sum){
	$('#title_s_w').html('??Yes??and??in??and? ?and????? in?in?Yes ?'+id)
	$('#img_val').val(img);
	imgWithdraw();
	$('#min_w').val(min_sum);
...
function editSystemDep(id, name, img, comm_percent, min_sum, ps, number_ps){
	$('#title_s_w').html('??Yes??and??in??and? ?and????? ????????and? ?'+id)
	$('#img_val').val(img);
	imgWithdraw();
	$('#min_w').val(min_sum);
	$('#comm_percent').val(comm_percent)
...
function closeEditSystemDep(argument) {
	$('#title_s_w').html('to??in???and? ?and???? ????????and?')
	$('#img_val').val('');
	imgWithdraw();
	$('#min_w').val('50');
	$('#comm_percent').val('5')
...
function editStatus(id, color, deposit, name, bonus){
	$('#title_s_w').html('??Yes??and??in??and? ??andinand???andand ?'+id)
	$('#id_status').val(id);
...
function closeEditStatus(argument) {
	$('#title_s_w').html('to??in???and? ??andinand???and?')
	$('#color_status').val('');
	$('#deposit_status').val('');
	$('#name_status').val('')
	$('#bonus_status').val('')
...
function editRepost(id, color, repost, bonus){
	$('#title_s_w').html('??Yes??and??in??and? ???in?? Repost? ?'+id)
	$('#id_repost').val(id);
...
function closeEditRepost() {
	$('#title_s_w').html('to??in???and? ???in?? Repost?')
	$('#color_repost').val('');
	$('#to_repost').val('');
	$('#bonus_repost').val('')
