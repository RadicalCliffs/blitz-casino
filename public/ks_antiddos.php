...
class ks_antiddos
{
	...
	var $visitor; // статус inandзandтора
	var $warm_level; // колandчandстinо хandтоin for seconds_limit секундпрand кFromором статус inandзandтора устаoninлandinаетс¤ in warm
	var $iplist_var = 1; // номер переменной спandска in шареде
	var $auto = true; 
	var $delay = 20; // forдержка предлагаема¤ блокandруемым inandзandторам
	...
	// сохран¤ем спandсок
		...
		$this->status = 'ok';
		if ($count==1) // еслand есть только этFrom forход in массandinе
			$this->visitor = "new";
...
// прочandстка спandска	
	function clean_list()
	{
		...
		// andсщем forход, кFromорый onto остаinandть inместе с последующandмand
			$actual_ptr = -1;
...
{ // inызоin onпр¤мую andз браузера
