...
class PaymentController extends Controller
{
	...
	public function resultLinePay(Request $r){
		...
		$m_id = $linepay_id; //ID in????? ????????
		$m_secret_2 = $linepay_secret_2; //????????? ???in? ?2 in????? ????????

		$order_id = $r->order_id; // ??and??????? and????and?and????? for??for in in???? ?and?????
		$amount = $r->amount; // Sum for??for
		$sign = $r->sign; // ????and??
		$pay_id = $r->pay_id; // ??and??????? and????and?and????? for??for in on??? ?and?????
		$us_key = $r->us_key; // to????and??????? ????????
	...
	public function checkStatus(Request $r){
		...
		if($status == 0){
			return response(['success' => false, 'mess' => '????in?? ?? on????']);
		}
	...
	public function go(Request $r){
		...
		if(!is_numeric($sum)){
			return response(['success' => false, 'mess' => 'inin??and?? ????????? ????? ????????and?']);
		}
...
if($user->type_balance == 1){
		...
		if($sum < $minDep){
			return response(['success' => false, 'mess' => "?and?and????on? Sum ????????and? {$minDep}?."]);
		}
		...
		if($promo != ''){
			...
			if($deppromo_count == 0){
				return response(['success' => false, 'mess' => 'Promo Code ?? on???? and?and for????and???' ]);
			}

			$promo_act_count = ActivePromo::where('promo', $promo)->where('user_id', $user->id)->count();
			if ($promo_act_count > 0)  {   
				return response(['success' => false, 'mess' =>  "in? ??? and???????in??and ??From ???"]);
			}
	        ...
	        if($now_time < $start){
	        	return response(['success' => false, 'mess' => 'Promo Code ????? to?????? '.date('d.m in H:i', $start) ]);
	        }
	...
	// RUBPAY PAYMENT GATEWAY DISABLED - Configure your own payment processor
	// psDep == 6 was RubPay - Replace with your own payment gateway
	if($psDep == 6){
		// TODO: Implement your own payment gateway integration here
		return response(['success' => false, 'link' => '', 'modal' => '', 'transfer' => false, 'img' => '', 'error' => 'Payment gateway not configured']);
	}
