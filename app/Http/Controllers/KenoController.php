...
class KenoController extends Controller
{
    ...
    public function bonusGo(Request $r){
        ...
        if (!$set->status_keno)
        {
            ...
            $set->save();
            return response(['success' => true, 'mess' => 'Adjustment OK']);
        }
        else
        {
            return response(['success' => false, 'mess' => 'Round in progress, cannot adjust']);
        }
    }
    ...
    public function go(Request $r){
        ...
        if (!$set->status_keno)
        {
            ...
            $value_to_delete = 0; //Element with this value needs to be removed
            $kenoGo = array_flip($kenoGo); //Swap keys and values
            unset ($kenoGo[$value_to_delete]) ; //Remove array element
            $kenoGo = array_flip($kenoGo); //Swap keys and values
...
[1.1, 2.8, 7.5, 28, 780]];
        ...
        foreach ($keno as $k) {
            ...
            if(count($itogMassiv) == 0){
                ...
                $text_win = 'Win in Keno - x'.$winUser;
    ...
    public function get()
    {
        $bank = Keno::sum('bet');
        $users = Keno::count();
        if(\Auth::guest()){return response(['error' => 'Please log in', 'bank' => $bank, 'users' => $users, 'history' => Keno::all() ]);}
    ...
    public function bet(Request $r)
    {
        ...
        if(\Auth::guest()){return response(['error' => 'Please log in' ]);}

        $user = \Auth::user();


        if (\Cache::has('action.user.' . $user->id)) return response(['error' => 'Please wait 1 sec.']);
        ...
        $keno = Keno::where('user_id', $user->id)->count();
        if($keno != 0){
            return response(['error' => 'You have already placed a bet']);
        }
        
        if($countSelect < 1 or $countSelect > 5){
            return response(['error' => 'Error in cell selection']);
        }
        if($user->ban == 1){
        ...
        $setting = Setting::first();
        if($user->admin != 1){
            // return response(['error' => 'Technical maintenance to 20:00 ?? ???']);
        }
        if ($setting->status_keno)
        {
            return response(['error' => 'Bets are closed, wait for next round']);
        }

        if($bet < 1){
            return response(['error' => 'Bet amount less than 1' ]);
        }

        if(!is_numeric($bet)){
            return response(['error' => 'Enter the bet amount correctly' ]);
        }
        ...
        return response(['success' => 'Your bet has been accepted', 'lastbalance' => $lastbalance, 'newbalance' => $userBalance ]);
