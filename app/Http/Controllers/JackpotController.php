...
class JackpotController extends Controller
{
    ...
    public function randNumber($min, $max)
    {
        ...
        $proxy_ip = '91.198.208.72:8000'; //Proxy server IP address and port
        $loginpassw = 'D28xH1:FvysGs'; //login and password for proxy
        $timeout = 5;
	...
	public function selectHunt(Request $request){
		...
		$user = Auth::user();
		if(Setting::first()->status_jackpot != 2) return response(['error'=>'Bonus game has not started or has ended!']);
		$count = Jackpot::where('user_id', $user->id)->count();
		if($count == 0){
			return response(['error'=>'You are not participating in this game']); 
		}
		if($id < 1 or $id > 64){
...
return response(['error' => 'An unknown error occurred']);
		...
		if(Setting::first()->status_jackpot) return response(['error'=>'Game has started or has ended!']);
		if(Jackpot::where(['user_id'=>$user->id])->count() >= 3) return response(['error'=>'Maximum 3 bets per round']);
		if($bet < 1) return response(['error'=>'Minimum bet is 1 coin']);
		if($bet > 10000) return response(['error'=>'Maximum bet is 10000 coins']);
		if($bet > $user->balance) return response(['error'=>'Insufficient funds']);
		if($user->deps < 99) return response(['error' => 'To place a bet you must have a minimum deposit amount of 99 rubles']);
		$jackpot = Jackpot::all();
		if(!$jackpot->count()){
