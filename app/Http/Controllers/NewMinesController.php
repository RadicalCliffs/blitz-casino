...
class NewMinesController extends Controller
{
	...
	public function click(Request $r){
		...
		if(in_array($mine, $click)){
			return response(['success' => false, 'mess' => 'You have already clicked this cell' ]);
		}
	...
	public function finish(){
		...
		$step = $cache_gameMine->step;
		if($step < 1){
			return response(['success' => false, 'mess' => 'You have not clicked any cell' ]);
		}
		$user->minesStart = 0;
		$user->save();
	...
	public function start(Request $r){
		...
		if (\Cache::has('action.user.' . $user->id)) return response(['success' => false, 'mess' => 'Please wait 1 sec.']);
        

		if($bet < 1){
			return response(['success' => false, 'mess' => 'Bet amount less than 1' ]);
		}
		$levels = [16, 25, 36, 49];
		...
		if(round($bomb) != $bomb ){
			return response(['success' => false, 'mess' => 'Enter a valid number of bombs' ]);
		}
		...
		if(!is_numeric($bet)){
			return response(['success' => false, 'mess' => 'Enter the bet amount correctly' ]);
		}
		if(!is_numeric($bomb)){
			return response(['success' => false, 'mess' => 'Enter a valid number of bombs' ]);
		}
		...
		if($games_on > 0){
			return response(['success' => false, 'mess' => 'You have active games' ]);
		}
