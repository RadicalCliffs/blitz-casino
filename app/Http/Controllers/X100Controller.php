...
class X100Controller extends Controller
{
    ...
    public function go(Request $request)
    {
        ...
        if (!$set->status_x100)
        {
            ...
            $set->win_x100 = $color;
            $set->save();
            return response(['success' => true, 'mess' => '????????? on x' . $color]);
        }
        else
        {
            return response(['success' => false, 'mess' => 'and??? ?????, ?????? ????and??']);
        }
    }
    ...
    public function bonusGo(Request $r){
        ...
        if (!$set->status_x100)
        {
            ...
            $set->save();
            return response(['success' => true, 'mess' => 'Bonus?? Success ????????on']);
        }
        else
        {
            return response(['success' => false, 'mess' => 'and??? ?????, ?????? ????and??']);
    ...
    public function generateNumber()
    {
      ...
      public function winWheel(Request $r){
        ...
        foreach ($wheel as $w) {
            ...
            $unmWin = 1;
            $text_win = 'in?and???? in X100 on x'.$wheelWinNumber;

            if($X100BonusUser_ID == $user_id){
                $unmWin = 4;
                $text_win = 'in?and???? in X100 on x'.$wheelWinNumber.'. Bonus 4x';
            }
    ...
    public function bet(Request $r)
    {
        ...
        //return response(['error' => '???and????? ??and?in???on? Error']);
        ...
        {
            return response(['error' => '???in?and for?????, ??and?? ???????and? ?????']);
        }

        if (\Cache::has('action.user.' . $user->id)) return response(['error' => '??to??and?? 1 ???.']);
        ...
        {
            return response(['error' => '???and????? Error']);
        }
        ...
        {
            return response(['error' => '????and??????? ???and????in? ???in?? 3']);
        }

        if ($bet < 1)
        {
            return response(['error' => '?and?and????on? ???in?? 1 ??No?']);
        }

        if ($bet > 100000)
        {
            return response(['error' => '????and????on? ???in?? 100000 ??No']);
        }
...
$userBalance = $user->type_balance == 0 ? $user->balance : $user->demo_balance;
        ...
        return response(['success' => 'in??? ???in?? ??and????', 'lastbalance' => $lastbalance, 'newbalance' => $userBalance, ]);
