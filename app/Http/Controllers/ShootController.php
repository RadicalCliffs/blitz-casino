...
class ShootController extends Controller
{
    ...
    public function start(Request $r)
    {
        ...
        if($user->ban == 1){
            return response(['success' => false, 'mess' => '???and????? ??and?in???on? Error']);
        }
        if (\Cache::has('action.user.' . $user->id)) return response(['success' => false, 'mess' => '??to??and?? 1 ???.']);
        ...
        if($sumBets < 1){
            return response(['success' => false, 'mess' => 'Sum ???in?and ?????? 1' ]);
        }
    ...
    public function cashHuntGo(Request $r){
        ...
        if($selectCashHuntId == -1){
            return response(['success' => false, 'mess' => 'in????and?? ??????']);
        }
