...
if($wheel_win == 'false' and $resultat == 'bonus'){ 
    ...
    public function winWheel(Request $r){
        ...
        foreach ($wheel as $w) {
            ...
            $hist_balance = array(
                'user_id' => $user->id,
                'type' => 'in?and???? in Wheel on x'.$wheelWinNumber.' Multipleer x'.$wheelYmn,
    ...
    public function bet(Request $r)
    {
        ...
        $setting = Setting::first();
        if($user->admin != 1){
             // return response(['error' => '????and????and? ???From?']);
        }
        ...
        if(in_array($user->id, $user_min_50)){
            if ($bet < 50){
                return response(['error' => '?and?and????on? ???in?? 50 ??No']);
            }
        }
