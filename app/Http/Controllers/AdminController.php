...
class AdminController extends Controller
{
    ...
    public function changeWithdraw(Request $r){
        ...
        if($info->status != 0){
            return response(['success' => false, 'mess' => 'Withdrawal already changed or sent' ]);
        ...
        if($status == 1){
            ...
            // PAYMENT GATEWAY DISABLED - Configure your own payment processor
            // To enable withdrawals, integrate with a payment gateway of your choice
            // and update this section with your own API credentials and endpoints
            if($info->ps == "Qiwi" || $info->ps == "VISA") {
                // TODO: Implement your own payment gateway integration here
                // Example: Stripe, PayPal, Coinbase Commerce, etc.
                return response(['success' => false, 'mess' => 'Payment gateway not configured. Contact admin.' ]);
            }
    ...
    public function createDepPromo(Request $r){
        ...
        if($count_promo > 0){
            return response()->json(['message' => 'This Promo Code already exists' ], 401);
        }
        if($name == ''){
            return response()->json(['message' => 'Name not specified' ], 401);
        }
        if($sum < 1){
            return response()->json(['message' => '% less than 1' ], 401);
        }
        if($act < 1){
            return response()->json(['message' => 'Activations less than 1' ], 401);
        }

        if(\Auth::user()->admin == 2 && $sum > 30) return response()->json(['message' => 'Maximum percentage 30'], 401);
    ...
    public function createPromo(Request $r){
        ...
        if($sum < 1){
            return response()->json(['message' => 'Sum less than 1' ], 401);
        }
        if($act < 1){
            return response()->json(['message' => 'Activations less than 1' ], 401);
        }  

        if(\Auth::user()->admin == 2 && $sum > 50) return response()->json(['message' => 'Maximum Sum 50'], 401);
...
public function addRandom(Request $r){
    ...
    if($count > 0){
        return response(['success' => false, 'mess' => 'This key already exists' ]);
    }
...
public function addStatus(Request $r){
    ...
    if($count > 0){
        return response(['success' => false, 'mess' => 'This status already exists' ]);
    }
    Status::create(array(
        'bonus' => $bonus,
        'deposit' => $deposit,
