...
class ChatController extends Controller
{
  ...
  public function ban(Request $r){
    ...
    if($this->user->admin == 0) return response(['success'=> false, 'mess' => 'Error']);
    if($why_ban == 0) return response(['success' => false, 'mess' => 'Specify ban reason']);
    ...
    if($time_chat_ban > $now) return response(['success'=>false,'mess'=>'User is already banned']);
    if($chat_ban == 1) return response(['success'=>false,'mess'=>'User is already banned']);
    ...
    if($time_ban == '') {
      ...
      $date_ban_do = date('to d.m H:i', $date_sec);
    }
    
    $w_w = ["begging", "spreading ref codes", "insults", "spam", "promo leaking", "PR", "slander", "misleading"];


    $message = Message::create([
      'content'  => "User ".$name." was blocked in Chat ".$date_ban_do.". \n Reason: ".$w_w[$why_ban - 1],
      'type_mess' => 4,
      'autor' => '<span style="color:#7001b2;">SYSTEM</span>',
  ...
  public function promoPublish1(){
    ...
    $message = Message::create([
      'content'  => $name,
      'type_mess' => 0,
      'autor' => '<span style="color:#e45151">New Promo Code</span>',
      'avatar' => "../img/ava_c.png",
      'user_id' => 0,
      'status_mess' => '',
      'time' => date('H:i')
  ...
  public function sendSticker(Request $request){
    ...
    if(\Cache::has('action.user.' . $this->user->id)) return response(['success' => false, 'mess' => 'Please wait 3 sec.']);
    \Cache::put('action.user.' . $this->user->id, '', 3);

    if($this->user->time_chat_ban > time()) return response(['success' => false, 'mess' => 'You are blocked in Chat until '. date("d.m.Y H:i", $this->user->time_chat_ban)]);
    if($this->user->chat_ban == 1){  return response(['success' => false, 'mess' => 'You are blocked in Chat forever' ]);}
    ...
    if($status_mess != 0) {
      ...
      $status_mess = '<span class="user-status new">Newbie</span>';
    }

    if($admin == 1){
      $ava = '../img/ava_c.png';
      $name_c = '<span style="color:#e45151">Administrator</span>';
      $status_mess = '';
    }

    if($admin == 2){
      $ava = '../img/ava_c.png';
      $name_c = '<span style="color:#33bf7c">Moderator</span>';
      $status_mess = '';
    }
  ...
  public function postMessage(Request $request){
    ...
    if(\Cache::has('chat.user.' . $this->user->id)) return response(['success' => false, 'mess' => 'Please wait before the previous action!' ]);
    \Cache::put('chat.user.' . $this->user->id, '', 2);

    if($this->user->time_chat_ban /* <= */ > time()) return response(['success' => false, 'mess' => 'You are blocked in Chat until '. date("d.m.Y H:i", $this->user->time_chat_ban)]);
    if($this->user->chat_ban == 1) return response(['success' => false, 'mess' => 'You are blocked in Chat forever' ]);
    if(!trim($mess)) return response(['success' => false, 'mess' => 'Enter a message' ]);
    if(strlen($mess) > 150) return response(['success' => false, 'mess' => 'Message length exceeds 150']);
    if(preg_match("/href|url|http|https|www|.ru|.com|.net|.info|csgo|winner|ru|xyz|com|net|space|site|fun|top|run|info|.org/i", $mess)) return response(['success' => false, 'mess' => 'Links are prohibited' ]);
    if(preg_match("/href|.org/i", $mess)) return response(['success' => false, 'mess' => 'Promo codes are prohibited' ]);
