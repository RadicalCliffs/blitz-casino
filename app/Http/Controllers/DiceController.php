...
class DiceController extends Controller
{
	...
	public function play(Request $r){
		...
		if($bet < 1){
			return response(['success' => false, 'mess' => 'Bet must be at least 1' ]);
		}
		if($user->ban == 1){
			return response(['error' => 'Server error']);
		}
		if(!is_numeric($bet)){
			return response(['success' => false, 'mess' => 'Invalid bet amount' ]);
		}

		if(!is_numeric($percent)){
			return response(['success' => false, 'mess' => 'Invalid percentage' ]);
		}

		if($percent < 1){
			return response(['success' => false, 'mess' => 'Percentage less than 1' ]);
		}

		if($percent > 95){
			return response(['success' => false, 'mess' => 'Percentage greater than 95' ]);
		}

		$userBalance = $user->type_balance == 0 ? $user->balance : $user->demo_balance;

		if($userBalance < $bet){
			return response(['success' => false, 'mess' => 'Insufficient balance' ]);
		}
