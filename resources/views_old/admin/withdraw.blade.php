					...
					<div class="flex no_padding wrap">
						<div class="col-lg-4">
							<div class="content-area " style="min-height: 10px;">
								<button class="btn-dep btn_w btn_w_all w-100" onclick="getWithdraws('all', 0)">inсе Withdrawalы</button>
							</div>
						</div>

						<div class="col-lg-4">
							<div class="content-area " style="min-height: 10px;" >
								<button class="btn-dep btn_w btn_w_0  w-100" onclick="getWithdraws(0, 0)">Актandinные Withdrawalы</button>
							</div>
						</div>

						<div class="col-lg-4">
							<div class="content-area " style="min-height: 10px;">
								<button class="btn-dep btn_w btn_w_1 w-100" onclick="getWithdraws(1, 0)">inыполненные Withdrawalы</button>
							</div>
						</div>

						<div class="col-lg-4">
							<div class="content-area " style="min-height: 10px;">
								<button class="btn-dep btn_w btn_w_mult w-100" onclick="getWithdraws('all', 1)">Withdrawalы мультоin</button>
							</div>
						</div>
					</div>
	...
	function getWithdraws(type = '', mult) {
		...
		$.post('/admin/withdraws/all',{_token: csrf_token, type, mult}).then(e=>{
			...
			e.withdraws.forEach((e)=>{
				...
				system = '';
				if(e.status  == 1){
					st = '<span class="badge badge_success">Fromпраinлен</span>';
					buttons = '';
				}else if(e.status == 0){
					st = '<span class="badge badge_warning">Pending</span>';
				}else if(e.status == 2){
					st = '<span class="badge badge_error">Fromменен</span>';
					buttons = '';
				}else{
					st = '<span class="badge badge_warning">Processing</span>';
				}
