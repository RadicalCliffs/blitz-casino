      ...
      <div class="card-body">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{date('d.m.y in H:i:s', strtotime($u->created_at))}}</td>
            <td id="btns_bun_id_{{$u->id}}"><a href="user/{{$u->id}}" class="btn btn-primary btn-sm me-2">Go</a>@if($u->ban == 1)<button onclick="changeBan({{$u->id}}, 0)" class="btn btn-success btn-sm ">Unblock</button>@else<button onclick="changeBan({{$u->id}}, 1)" class="btn btn-danger btn-sm">Block</button>@endif</td>
        </tr>
        @endforeach
