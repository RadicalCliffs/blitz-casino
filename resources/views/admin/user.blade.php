...
<div class="row">
    <div class="col-xl-4">
              ...
              <div class="profile-title">
                <div class="media" style="align-items: center;">      
                    ...
                    <div class="media-body ms-3">
                        ...
                        <p>@if($user->admin == 1) Administrator @else User @endif</p>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div class="mb-3">
          ...
          <input class="form-control" disabled value="{{$user->status == 0 ? 'Newbie' : ($user->status == 1 ? 'Wolf' : ($user->status == 2 ? 'Predator' : ($user->status == 3 ? 'Premium' : ($user->status == 4 ? 'Alpha' : ($user->status == 5 ? 'VIP' : ($user->status == 6 ? 'Pro' : 'Legend'))))))}}">
      </div>
      <div class="mb-3">
      ...
      <div class="mb-3">
          ...
          <input class="form-control" disabled value="{{date('d.m.y in H:i:s', strtotime($user->created_at))}}">
      </div>


      <div class="row">
        <div class="col-6">
            @if($user->ban == 1)<button type="button" onclick="changeBan({{$user->id}}, 0)" class="btn btn-success w-100">Unblock</button>@else<button type="button" onclick="changeBan({{$user->id}}, 1)" class="btn btn-danger w-100">Block</button>@endif
        </div>
        <div class="col-6"><button class="btn btn-danger w-100" type="button" onclick="deleteUser({{$user->id}})">Delete account</button></div>
    </div>
  ...
  <div class="card-body">
      <div class="row">
        <div class="col-md-6">
            <div class="mb-3">
                <label class="form-label">Balance</label>
                <input class="form-control" type="text" value="{{$user->balance}}" id="balance" placeholder="Balance">
            </div>
        </div>
        <div class="col-md-6">
            <div class="mb-3">
                <label class="form-label">Demo Balance</label>
                <input class="form-control" type="text" value="{{$user->demo_balance}}" id="demo_balance" placeholder="Balance">
            </div>
        </div>
    ...
    <div class="col-md-6">
      <div class="mb-3">
        ...
        <select class="form-control" id="admin" value="{{$user->admin}}"> 
            ...
            <option value="1" {{ $user->admin == 1 ? 'selected' : ''}}>Administrator</option>                               
            <option value="2" {{ $user->admin == 2 ? 'selected' : ''}}>Moderator</option> 
...
<div class="card-footer text-end">
  <button class="btn btn-primary" onclick="saveUser({{$user->id}})" type="button">Save</button>
</div>
</form>
<div class="card">
    <div class="card-header">
...
<table class="table "  style="margin-bottom: 20px;"> 
  ...
  <tbody>
      ...
      <tr>
          ...
          <td>{{date('d.m.y in H:i:s', strtotime($acc->created_at))}}</td>
          <th scope="col">@if($acc->ban == 0)<button onclick="changeBan({{$acc->id}}, 1)" class="btn btn-info btn-sm">Block</button> @else<button onclick="changeBan({{$acc->id}}, 0)" class="btn btn-info btn-sm">Unblock</button> @endif</th>
      ...
      <table class="table "  style="margin-bottom: 20px;"> 
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{date('d.m.y in H:i:s', strtotime($d->created_at))}}</td>

                <th scope="col">@if($d['status'] == 0)<button onclick="changePay({{$d->id}})" class="btn btn-info btn-sm">Credit deposit</button>@endif</th>
      ...
      <table class="table "  style="margin-bottom: 20px;"> 
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{date('d.m.y in H:i:s', strtotime($w->created_at))}}</td>

                <th scope="col">@if($w['status'] == 0)<button onclick="changeWithdraw({{$w->id}}, 1)" class="btn btn-info btn-sm">Withdraw</button>@endif</th>
...
<div class="col-md-12">
  <div class="card">
    <div class="card-header">
      <h4 class="card-title mb-0">Balance History</h4>
      <div class="card-options"><a class="card-options-collapse" href="#" data-bs-toggle="card-collapse"><i class="fe fe-chevron-up"></i></a><a class="card-options-remove" href="#" data-bs-toggle="card-remove"><i class="fe fe-x"></i></a></div>
  </div>
  <div class="table-responsive add-project">
      ...
      <table class="table "  style="margin-bottom: 20px;"> 
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{date('d.m.y in H:i:s', strtotime($h->date))}}</td>
