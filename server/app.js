...
// Import Blitz VRF for verifiable fairness
const BlitzVRF = require('./vrf');

// Use native https module for requests instead of node-libcurl
const https = require('https');
const http = require('http');

// Local development configuration
const isLocalDev = process.env.NODE_ENV !== 'production';
domain = isLocalDev ? 'http://localhost:8000' : 'https://blitz.casino';

var crypto = require('crypto'); 

// Use mysql2 for MySQL 9.x compatibility (supports caching_sha2_password)
...
// Helper function to make HTTP requests (replaces requestify)
function makeRequest(url, method = 'GET', data = null) {
    return new Promise((resolve, reject) => {
        ...
        const req = requester.request(options, (res) => {
            ...
            res.on('end', () => resolve({ body, getCode: () => res.statusCode }));
        });
        
        req.on('error', (err) => reject({ message: err.message, getCode: () => 0 }));
...
if (isLocalDev) {
    ...
    server = require("https").createServer({
        key: fs.readFileSync('/etc/letsencrypt/live/blitz.casino/privkey.pem'),
        cert: fs.readFileSync('/etc/letsencrypt/live/blitz.casino/fullchain.pem')
    }, app);
}

const io = require("socket.io")(server, {
    cors: {
        origin: isLocalDev ? ["http://localhost:8000", "http://127.0.0.1:8000"] : "https://blitz.casino",
        methods: ["GET", "POST"]
    }
});


// Express routes for VRF verification
app.get('/api/vrf/config', (req, res) => {
    res.json(BlitzVRF.getPublicConfig());
});

app.get('/api/vrf/verify', async (req, res) => {
    const { seed, signature, value } = req.query;
    if (!seed || !signature || !value) {
        return res.status(400).json({ error: 'Missing parameters' });
    }
    const isValid = await BlitzVRF.verifyResult(seed, signature, parseInt(value));
    res.json({ valid: isValid });
});

const SERVER_PORT = process.env.SOCKET_PORT || 2083;
server.listen(SERVER_PORT, () => {
    console.log(`[Blitz] Socket server running on port ${SERVER_PORT}`);
    console.log(`[Blitz] VRF Config:`, BlitzVRF.getPublicConfig());
});
...
function x100_bot() {
    console.log(domain)
    makeRequest(domain+'/x100bot', 'GET')
    .then(({ body }) => {
        console.log('responsebody', body);
        console.log('responsebody RAW', body);
    })
    .catch((err) => {
...
async function randOrgJackpot(){
    await makeRequest(domain+'/generate_jackpotnumber', 'GET')
    .then(async ({ body }) => {
        console.log(body)
        return 'True'
    })
...
async function randOrg(game) {
 await makeRequest(domain+'/generate_number_'+game, 'GET')
 .then(async ({ body }) => {
    console.log(body)
...
async function winUserX100(){
  await makeRequest(domain+'/winx100', 'GET')
  .then(async ({ body }) => {
    console.log(body)
    return 'True'
})
...
async function winUserWheel(){
    await makeRequest(domain+'/winwheel', 'GET')
    .then(async ({ body }) => {
        console.log(body)
        return 'True'
    })
...
async function cashHuntFinish(){
    await makeRequest(domain+'/cashhuntfinish', 'GET')
    .then(async ({ body }) => {
        console.log(body)
        return 'True'
    })
...
async function winUserKeno(){
    await makeRequest(domain+'/winkeno', 'GET')
    .then(async ({ body }) => {
        console.log(body)
        return 'True'
    })
...
async function winUserCrash() {
    // Using native http for local development
    return new Promise((resolve, reject) => {
        const url = new URL(domain + '/wincrash');
...
async function promoPublish(time){
    makeRequest(domain+'/chat/promo/publish', 'POST')
...
function sendPromo() {
	...
	.catch((err) => {
		log.error('[CHAT] Error ??and ???????andand ???????Yes!');
